...
local Library = loadstring(game:HttpGet("https://pastebin.com/raw/TYcpsFEi"))()
...
spawn(function()
	...
	local OldCharacter, NewCharacter
	local Loop, Loop2 = nil, nil
	local TweenTo = nil
	local DamageFlight = Tabs.Move:CreateToggle({
		Name = "Damage Flight",
		Callback = function(callback)
			if callback then
					...
					Loop = Service.RunService.RenderStepped:Connect(function()
						if IsAlive(OldCharacter) then
							if not NewCharacter then
								NewCharacter = CloneMe(OldCharacter)
								NewCharacter.Parent = game.Workspace
								NewCharacter.HumanoidRootPart.CFrame = CFrame.new(NewCharacter.HumanoidRootPart.Position, NewCharacter.HumanoidRootPart.Position + OldCharacter.HumanoidRootPart.CFrame.LookVector) 
							end
							if IsAlive(NewCharacter) then
								local NewVelo = NewCharacter:FindFirstChildOfClass("Humanoid").MoveDirection * 100
								game.Workspace.CurrentCamera.CameraSubject = NewCharacter:FindFirstChildOfClass("Humanoid")
								LocalPlayer.Character = NewCharacter
								NewCharacter:FindFirstChild("HumanoidRootPart").Velocity = Vector3.new(NewVelo.X, 2.5, NewVelo.Z)
							end
							if not Loop2 then
								Loop2 = OldCharacter:FindFirstChildOfClass("Humanoid"):GetPropertyChangedSignal("Health"):Connect(function()
									if not AlreadyDamaged then
										TweenTo = game:GetService("TweenService"):Create(OldCharacter.HumanoidRootPart, TweenInfo.new(2.2), {CFrame = NewCharacter.HumanoidRootPart.CFrame - NewCharacter.HumanoidRootPart.CFrame.LookVector * 3})
										if TweenTo then
											TweenTo:Play()
											TweenTo.Completed:Connect(function()
												AlreadyDamaged = true
												game:GetService("StarterGui"):SetCore("SendNotification", { 
													Title = "Lime | Damage Flight",
													Text = "You can disable the flight now.",
													Duration = 3
												})
											end)
										end
